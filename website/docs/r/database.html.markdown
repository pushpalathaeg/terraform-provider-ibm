...
- `async_restore` - Should only be set when promoting a asynchronously restore. By setting this value to `true`, restore first member synchronously, then start restoring second member asynchronously. The default is `false`.
